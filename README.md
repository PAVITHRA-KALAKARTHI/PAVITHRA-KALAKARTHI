# Hi 👋, I'm Pavithra K

### 🚀 Aspiring Software Engineer | AI & Data Science Student

🎓 B.Tech in Artificial Intelligence and Data Science  
🏫 Kongu Engineering College (2023 - 2027)  
📈 CGPA: 8.07 (Till 5th Semester)

---

## 👩‍💻 About Me

I am an enthusiastic AI & Data Science student passionate about software development, web technologies, data analysis, and machine learning. I enjoy building scalable applications, solving real-world problems, and continuously improving my technical skills.

---

## 🛠️ Technical Skills

### Programming Languages
- Java
- Python
- C

### Web Development
- HTML
- CSS
- Bootstrap
- JavaScript
- React.js

### Backend Development
- Node.js
- Express.js
- REST APIs

### Databases
- SQL
- MongoDB

### Tools & Technologies
- Git
- GitHub
- Power BI

### Core Concepts
- Data Structures and Algorithms
- Problem Solving

---

## 🚀 Projects

### 🔹 Work Finder App for Blue-Collar Labourers
- Developed a scalable full-stack web application.
- Built using React, Node.js, Express.js, and MongoDB.
- Implemented secure authentication and REST APIs.
- Added real-time job tracking features.

### 🔹 Duty Schedule Allotment System
- Developed a Java-based scheduling system.
- Used MySQL and JDBC for database connectivity.
- Automated employee shift allocation.
- Reduced manual scheduling errors.

### 🔹 Fake Job Posting Detection
- Implemented a Machine Learning model.
- Classified job postings as Fake or Genuine.
- Applied feature engineering and model evaluation techniques.

### 🔹 Cashflow Forecasting Dashboard
- Designed an interactive Power BI Dashboard.
- Used DAX calculations for financial analysis.
- Analyzed cash flow trends and forecasting insights.

---

## 🏆 Certifications

- Microsoft Azure AI-102 Associate Developer
- Oracle Java SE 17 Developer
- Oracle APEX Cloud Developer

---

## 🎯 Areas of Interest

- Data Analysis
- Machine Learning
- Software Development
- Full Stack Development

---

## 🌐 Connect With Me

📧 Email: pavithra.kalakarthi@gmail.com

💼 LinkedIn: https://linkedin.com/in/pavithra-k-502519335/

💻 GitHub: https://github.com/PAVITHRA-KALAKARTHI

🧩 LeetCode: https://leetcode.com/u/PAVITHRA_KARTHI/

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=PAVITHRA-KALAKARTHI&show_icons=true)


---

### 💡 Career Objective

To start my career as a Software Engineer where I can apply my skills in Java, web development, and problem-solving to build scalable applications and continuously improve my technical expertise.
